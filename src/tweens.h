...
protected:
  void aux_reset(anim_time_t n_duration, anim_time_t n_cur_time) {
    assert(n_cur_time > INT16_MIN / 2); assert(n_cur_time < INT16_MAX / 2);
    assert(n_duration >= 0); assert(n_duration < INT16_MAX / 2);
...
public:
  ...
  void reset_from_value(T n_from, T n_to, anim_time_t n_duration,
      T value) {
    m_from = n_from;
    m_delta = n_to - n_from;

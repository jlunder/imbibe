...
bool menu_handler::handle(int c) {
  ...
  x = m_me.scroll_x();
  y = m_me.scroll_y();
  if(x + m_me.frame_width()
      < m_m.link_x(selection) + m_m.link_normal_picture(selection).width()) {
    x = m_m.link_x(selection) + m_m.link_normal_picture(selection).width()
      - m_me.frame_width();
  } if(x > m_m.link_x(selection)) {
    x = m_m.link_x(selection);
  } if(y + m_me.frame_height()
      < m_m.link_y(selection) + m_m.link_normal_picture(selection).height()) {
    y = m_m.link_y(selection) + m_m.link_normal_picture(selection).height()
      - m_me.frame_height();

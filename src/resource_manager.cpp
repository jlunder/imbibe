  ...
  if(i == s_loaded_resources.end()) {
    ...
    if (!entry.resource) {
      ...
      logf_resource_manager("  header = %p, .name = %s, .index = %u\n",
        header, name.c_str(), (unsigned)index);
    ...
    i = s_loaded_resources.insert(
      loaded_resources_map::value_type(name,
        im_ptr<bitmap>(reclaim_loaded_data, (bitmap *)entry.resource)));
...
void resource_manager::load_data_at_hash(imstring const & name, uint16_t hash, uint16_t hint) {
  ...
  if (s_first_free_index == s_empty_index) {
    ...
    uint16_t target = min<uint16_t>(s_hash_capacity / 2,
    ...
    err);
  {
    ...
    if (size > (INT16_MAX / 2)) goto fail_return;
    entry.size = (uint16_t)size;
    entry.data = ::malloc(entry.size);
    assert(entry.data != NULL);
    if (entry.data == NULL) goto fail_return;
  }
